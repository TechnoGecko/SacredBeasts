...
namespace Utilities
{
    ...
    public sealed class SetupDemoLayers : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            Physics2D.SetLayerCollisionMask(DemoHitLayer, PlatformerGameKit.HitTrigger.HitLayers);
        }
        ...
        {
            var readMe = Animancer.Editor.AnimancerEditorUtilities.FindAssetOfType<Utilities.ReadMe>();
            if (readMe == null)
            {
                Debug.LogWarning($"Can't automatically swap existing objects to the newly created layers" +
                    $" because the {nameof(Utilities.ReadMe)} asset wasn't found (it is used to determine the base folder to modify assets in).");
                return;
            }
